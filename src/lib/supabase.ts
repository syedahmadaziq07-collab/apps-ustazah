...
if (isSupabaseConnected) {
  console.log('[Supabase] Client created — URL:', supabaseUrl.substring(0, 20) + '...', 'Key: exists(' + Boolean(supabaseAnonKey) + ')');
} else {
  console.warn('[Supabase] Client NOT created — URL:', Boolean(rawUrl?.trim()), 'Key:', Boolean(rawKey?.trim()));
}
